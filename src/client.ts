...
export class FakeMediaClient {
  ...
  request<T = unknown>(method: Parameters<typeof this.transport.request>[0], path: string, options?: Parameters<typeof this.transport.request>[2]) {
    return this.transport.request<T>(method, path, options);
  }

  get<T = unknown>(path: string, options?: Parameters<typeof this.transport.get>[1]) {
    return this.transport.get<T>(path, options);
  }

  post<T = unknown>(path: string, body?: unknown, options?: Parameters<typeof this.transport.post>[2]) {
    return this.transport.post<T>(path, body, options);
  }

  put<T = unknown>(path: string, body?: unknown, options?: Parameters<typeof this.transport.put>[2]) {
    return this.transport.put<T>(path, body, options);
  }

  patch<T = unknown>(path: string, body?: unknown, options?: Parameters<typeof this.transport.patch>[2]) {
    return this.transport.patch<T>(path, body, options);
  }

  delete<T = unknown>(path: string, options?: Parameters<typeof this.transport.delete>[1]) {
    return this.transport.delete<T>(path, options);
  }
}

export function createFakeMediaClient(options: ClientOptions = {}): FakeMediaClient {
